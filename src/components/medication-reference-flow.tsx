...
function Candidate({
  ...
  return (
    <li className="space-y-4 rounded-xl border bg-white p-4">
      <div className="space-y-1">
        <h2 className="font-semibold">{candidate.confirmedProductName}</h2>
      ...
      <a className="inline-flex min-h-11 items-center font-semibold text-[var(--accent)] underline" href={candidate.sourceUrl} rel="noreferrer" target="_blank">
...
function ConfirmedReference({ reference }: { reference: MedicationReference }) {
  ...
  return (
    <section className="space-y-4 rounded-xl border border-[var(--success)] bg-white p-4" aria-labelledby="official-reference-heading">
      <p className="text-sm font-semibold text-[var(--success)]">Official reference confirmed</p>
      <h2 className="text-lg font-semibold" id="official-reference-heading">{reference.officialTitle}</h2>
      ...
      <a className="inline-flex min-h-12 items-center rounded-xl bg-[var(--accent)] px-4 text-sm font-semibold text-white" href={reference.sourceUrl} rel="noreferrer" target="_blank">
        Read official information
      </a>
    </section>
  );
  ...
  return (
    <div className="space-y-6">
      {reference?.status === "unavailable" || unavailableState.status === "unavailable" ? (
        <section className="space-y-3 rounded-xl border bg-white p-4" aria-labelledby="unavailable-heading">
      ...
      {!searchState.lookupComplete ? (
        <form action={searchAction} className="space-y-4 rounded-xl border bg-white p-4" noValidate>
          ...
          <label className="flex min-h-12 items-start gap-3 text-sm font-medium">
            <input className="mt-1" name="lookupConsent" required type="checkbox" value="yes" />
            <span>I agree to this one-time official-source lookup.</span>
          </label>
          {searchState.error ? <p className="text-sm font-medium text-[var(--error)]" role="alert">{searchState.error}</p> : null}
      ...
      {searchState.lookupComplete || advancedState.lookupComplete ? (
        <section className="space-y-4" aria-labelledby="candidate-heading">
          ...
          {(searchState.candidates?.length || advancedState.candidates?.length) ? (
            ...
            <p className="rounded-xl border bg-white p-4 text-sm text-[var(--muted-foreground)]">No candidate matched the selected form or route. We did not save a match.</p>
          )}
      ...
      <details className="rounded-xl border bg-white p-4">
        <summary className="min-h-11 cursor-pointer py-2 font-semibold">Advanced: add an official DailyMed record</summary>
        <form action={advancedAction} className="mt-4 space-y-3" noValidate>
          ...
          {advancedState.error ? <p className="text-sm font-medium text-[var(--error)]" role="alert">{advancedState.error}</p> : null}
